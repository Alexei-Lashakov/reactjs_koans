...
class GroceryList extends React.Component {
  ...
  toggleGroceryCompleteness(groceryIndex) {
    let completedItem = this.state.groceries[groceryIndex];
    completedItem.completed = !completedItem.completed;
    this.setState({groceries: this.state.groceries.splice(groceryIndex, 1, completedItem)});
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        <NewProduct onChange={this.inputChanged} onClick={this.addGroceryItem} disabled={!this.state.newGroceryName} />
        {clearListButton}
      </div>
    );
  }
...
class NewProduct extends React.Component {
  render() {
    return (
      <div>
        <input className='new-item' type="text" onChange={this.props.onChange} />
        <button className='add-product' onClick={this.props.onClick} disabled={this.props.disabled}>Add new Product</button>
      </div>
    );
  }
}
